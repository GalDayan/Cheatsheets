 ...
 * @since      1.0.0
 */
class enjoyblog_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoyblog-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'enjoyblog' )
		);

		// Create the widget.
		parent::__construct(
			'enjoyblog-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'enjoyblog' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'enjoyblog_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoyblog_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoyblog_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'enjoyblog' ),
			'limit'     => 5,
			'show_date' => true
		);
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>">
				<?php esc_html_e( 'Title', 'enjoyblog' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'limit' ) ); ?>">
				<?php esc_html_e( 'Number of posts to show', 'enjoyblog' ); ?>
			</label>
		...
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'show_date' ) ); ?>">
				<?php esc_html_e( 'Display post date?', 'enjoyblog' ); ?>
			</label>
		</p>
