 ...
 * @since      1.0.0
 */
class enjoyblog_Random_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoyblog-random widget-posts-thumbnail',
			'description' => __( 'Display random posts with thumbnails.', 'enjoyblog' )
		);

		// Create the widget.
		parent::__construct(
			'enjoyblog-random',                                   // $this->id_base
			__( '&raquo; Random Posts', 'enjoyblog' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $random = get_transient( 'enjoyblog_random_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoyblog_random_widget_' . $this->id, $random );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoyblog_random_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Random Posts', 'enjoyblog' ),
			'limit'     => 5,
			'show_date' => true
		);
